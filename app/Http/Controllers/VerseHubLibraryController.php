...
class VerseHubLibraryController extends Controller
{
    public function index(Request $request, string $lang)
    {
        ...
        if ($s !== '') {
            ...
            // Only supported on SQLite (FTS5 virtual table).
            if (\DB::getDriverName() === 'sqlite') {
                // Escape quotes for MATCH query.
                $match = str_replace('"', '""', $term);

                try {
                    // Use bm25 ranking. Join back to bible_verses by rowid.
                    // NOTE: highlighting is done in Blade for simplicity.
                    $rows = \DB::select(
                        "SELECT rowid, book_code, chapter, verse, reference, text, bm25(bible_verses_fts) as rank
                         FROM bible_verses_fts
                         WHERE provider = 'ayt' AND lang = ? AND bible_verses_fts MATCH ?
                         ORDER BY rank
                         LIMIT {$limit}",
                        [$dbLang, $match]
                    );
                    ...
                    // If FTS table doesn't exist yet, just return no results.
                    $searchResults = [];
                }
            }
        }
