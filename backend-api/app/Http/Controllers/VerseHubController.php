...
class VerseHubController extends Controller
{
    ...
    {
        $baseUrl = rtrim((string) config('services.english_bible.base_url', 'https://bible-api.com'), '/');
        $translation = trim((string) config('services.english_bible.translation', ''));
        $apiKey = trim((string) config('services.english_bible.api_key', ''));
        $authMode = Str::lower(trim((string) config('services.english_bible.auth_mode', 'bearer')));
        $url = $baseUrl.'/'.rawurlencode($query);
        if ($translation !== '') {
            $url .= '?translation='.rawurlencode($translation);
        }

        return Cache::remember(
            'versehub:english-provider:'.md5($url.'|'.$authMode.'|'.($apiKey !== '' ? 'key' : 'nokey')),
            now()->addHours(6),
            function () use ($url, $apiKey, $authMode) {
                $http = Http::timeout(12);
                if (app()->environment('local')) {
                    $http = $http->withoutVerifying();
                }
