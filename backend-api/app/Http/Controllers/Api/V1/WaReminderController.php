...
class WaReminderController extends Controller
{
    ...
    private const SETTINGS_KEY = 'wa.settings';
    private const TEMPLATES_KEY = 'wa.templates';
    ...
    public function sendReminder(Request $request): JsonResponse
    {
        ...
        if (trim((string) $client->fonnte_token) === '') {
            WaLog::query()->create([
                'wa_client_id' => $client->id,
                'status' => 'client_token_missing',
                'response' => $this->encodeResponse([
                    'message' => 'Client token missing',
                ]),
            ]);
        ...
        foreach ($validated['rows'] as $index => $row) {
            ...
            if ($sentSameHash) {
                $results[] = $this->skipRow($client, $rowNumber, $name, $phone, $toko, $messageTemplate, $effectiveTimezone, $scheduleAt, 'sudah terkirim untuk revisi ini');
                continue;
            }

            // Kolom sheet mungkin masih menunjukkan "Terkirim"/id lama.
            // Selama sourceHash berubah (tanggal/jam/pesan/toko berubah), kita anggap revisi baru dan tetap kirim.
            if ($status === self::STATUS_TERKIRIM || $sendId !== '') {
                WaLog::query()->create([
                    'wa_client_id' => $client->id,
                    ...
                    'response' => $this->encodeResponse([
                        'reason' => 'sheet_status_or_id_ignored_due_to_revision_change',
                        'sheet_status' => $status,
                        'sheet_id_kirim' => $sendId,
                    ]),
                ]);
            }
    ...
    public function updateTemplate(Request $request, string $id): JsonResponse
    {
        ...
        $templates = $this->getTemplates();
        $found = false;
        $setDefault = (bool) ($validated['is_default'] ?? false);
        ...
        foreach ($templates as &$item) {
            ...
            $item['content'] = trim((string) $validated['content']);
            $item['is_default'] = $setDefault;
            $found = true;
        }
        unset($item);
    ...
    {
        $templates = $this->getTemplates();
        $filtered = array_values(array_filter($templates, fn ($item) => (string) ($item['id'] ?? '') !== $id));
    ...
    public function updateSettings(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'demo_max_reminders' => ['sometimes', 'integer', 'min:1', 'max:5000'],
            'demo_allowed_numbers' => ['sometimes', 'array'],
            'demo_allowed_numbers.*' => ['string', 'max:32'],
            'sender_label' => ['sometimes', 'nullable', 'string', 'max:120'],
            'timezone_default' => ['sometimes', 'nullable', 'string', 'max:64'],
        ]);

        $settings = array_merge($this->getSettingsPayload(), $validated);
        if (isset($settings['demo_allowed_numbers']) && is_array($settings['demo_allowed_numbers'])) {
            $settings['demo_allowed_numbers'] = array_values(array_unique(array_map(
                fn ($v) => trim((string) $v),
                $settings['demo_allowed_numbers']
            )));
        }

        $this->persistJsonSetting(self::SETTINGS_KEY, $settings);
    ...
    /**
     * @return array<int, array{id:string,name:string,content:string,is_default:bool}>
     */
    private function getTemplates(): array
    {
        $raw = $this->readJsonSetting(self::TEMPLATES_KEY, []);
        if (! is_array($raw)) {
            return [];
        }
        ...
        foreach ($raw as $item) {
            ...
            $normalized[] = [
                'id' => trim((string) ($item['id'] ?? (string) Str::ulid())),
                'name' => $name,
                'content' => $content,
        ...
        if (count($normalized) === 0) {
            return [[
                ...
                'content' => self::DEFAULT_MESSAGE_TEMPLATE,
                'is_default' => true,
            ]];
        }
    ...
    {
        $this->persistJsonSetting(self::TEMPLATES_KEY, array_values($templates));
    }
    ...
    private function getSettingsPayload(): array
    {
        $defaults = [
            'demo_mode' => false,
            'demo_max_reminders' => 10,
            'demo_allowed_numbers' => [],
            'sender_label' => 'TheChosenTalks',
            'timezone_default' => self::DEFAULT_CLIENT_TIMEZONE,
        ];

        $stored = $this->readJsonSetting(self::SETTINGS_KEY, []);
        if (! is_array($stored)) {
            return $defaults;
        }
    ...
    private function readJsonSetting(string $key, mixed $fallback): mixed
    {
    ...
    private function persistJsonSetting(string $key, array $payload): void
    {
