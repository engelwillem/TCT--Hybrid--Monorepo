...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->bind(CrmSyncAdapterInterface::class, function () {
            return strtolower((string) config('onboarding.integrations.mode', 'mock')) === 'webhook'
    ...
    {
        RateLimiter::for('whatsapp-otp-request', function (Request $request) {
            $userId = (string) optional($request->user())->id;
            $ip = (string) $request->ip();

            return [
                Limit::perMinute(5)->by("wa-otp-request:user:{$userId}"),
                Limit::perMinute(12)->by("wa-otp-request:ip:{$ip}"),
            ];
        });

        RateLimiter::for('whatsapp-otp-verify', function (Request $request) {
            $userId = (string) optional($request->user())->id;
            $ip = (string) $request->ip();

            return [
                Limit::perMinute(10)->by("wa-otp-verify:user:{$userId}"),
                Limit::perMinute(20)->by("wa-otp-verify:ip:{$ip}"),
            ];
        });

        RateLimiter::for('aios-intake', function (Request $request) {
            $ip = (string) $request->ip();

            return [
                Limit::perMinute(6)->by("aios-intake:ip:{$ip}"),
                Limit::perHour(30)->by("aios-intake-hour:ip:{$ip}"),
            ];
        });
