...
async function deploy() {
    ...
    console.log(`📦 [3/4] Menulis metadata build SHA dan membungkus artifact ${ARCHIVE_NAME}...`);
    try {
        const commitSha = execSync('git rev-parse HEAD', { stdio: ['ignore', 'pipe', 'pipe'] }).toString().trim();
        const commitShort = execSync('git rev-parse --short HEAD', { stdio: ['ignore', 'pipe', 'pipe'] }).toString().trim();
        ...
        fs.writeFileSync(path.join(__dirname, BUILD_INFO_NAME), JSON.stringify(buildInfo, null, 2));
        ...
        console.log(`✅ build info written: ${BUILD_INFO_NAME} (${commitShort})`);
    } catch (e) {
