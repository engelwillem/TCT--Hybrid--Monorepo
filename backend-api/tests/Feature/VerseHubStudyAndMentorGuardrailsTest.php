...
class VerseHubStudyAndMentorGuardrailsTest extends TestCase
{
    ...
    {
        parent::setUp();
        RateLimiter::clear('versehub:mentor:ask:guest');
        RateLimiter::clear('versehub:mentor:ask:1');
    }
    ...
    public function test_mentor_ask_is_rate_limited_after_ten_requests_per_hour(): void
    {
        ...
        RateLimiter::clear('versehub:mentor:ask:'.$user->id);
    ...
    public function test_study_path_progress_uses_last_step_order_and_marks_completion_on_final_step(): void
    {
        ...
        $this->postJson("/api/v1/study-paths/id/{$path->slug}/complete/{$step1->id}")
            ->assertOk()
        ...
        $this->postJson("/api/v1/study-paths/id/{$path->slug}/complete/{$step2->id}")
            ->assertOk()
            ->assertJsonPath('ok', true);
