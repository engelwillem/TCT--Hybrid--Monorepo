...
SKIP_HTTP="0"
HTTP_OK_CODES="${HTTP_OK_CODES:-200 301 302 303 307 308}"
...
for path in $URLS; do
    ...
    if ! grep -Eq "(^| )${code}( |$)" <<<"$HTTP_OK_CODES"; then
        echo "HTTP healthcheck failed: ${BASE_URL}${path} -> $code (allowed: $HTTP_OK_CODES)"
        exit 1
    fi
done
