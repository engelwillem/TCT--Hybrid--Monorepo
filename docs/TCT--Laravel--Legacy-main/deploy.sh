...
MIGRATE_SQLITE_TO_MYSQL_ON_DEPLOY="${MIGRATE_SQLITE_TO_MYSQL_ON_DEPLOY:-0}"
MIGRATE_SQLITE_SOURCE_CONNECTION="${MIGRATE_SQLITE_SOURCE_CONNECTION:-sqlite_legacy}"
MIGRATE_SQLITE_TARGET_CONNECTION="${MIGRATE_SQLITE_TARGET_CONNECTION:-mysql}"
MIGRATE_SQLITE_EXTRA_ARGS="${MIGRATE_SQLITE_EXTRA_ARGS:---truncate --exclude=migrations}"
...
if [[ "$DB_CONNECTION_VALUE" == "sqlite" ]]; then
    SQLITE_SHARED_FILE="${DB_DATABASE_VALUE:-$SHARED_DIR/database/database.sqlite}"
    mkdir -p "$(dirname "$SQLITE_SHARED_FILE")"
    [[ -f "$SQLITE_SHARED_FILE" ]] || touch "$SQLITE_SHARED_FILE"
    rm -f "$NEW_RELEASE/database/database.sqlite"
    ln -sfn "$SQLITE_SHARED_FILE" "$NEW_RELEASE/database/database.sqlite"
fi
...
if [[ "$DB_CONNECTION_VALUE" == "mysql" || "$DB_CONNECTION_VALUE" == "mariadb" ]]; then
    ...
    php -r 'require "vendor/autoload.php"; $app = require "bootstrap/app.php"; $app->make(Illuminate\Contracts\Console\Kernel::class)->bootstrap(); try { Illuminate\Support\Facades\DB::connection()->getPdo(); echo "MySQL connectivity OK\n"; } catch (Throwable $e) { fwrite(STDERR, "MySQL connectivity check failed: " . $e->getMessage() . "\n"); exit(1); }'
fi
...
log "Syncing admin account parity"
php artisan app:sync-admin-account

if [[ "$DB_CONNECTION_VALUE" == "mysql" || "$DB_CONNECTION_VALUE" == "mariadb" ]]; then
    if [[ "$MIGRATE_SQLITE_TO_MYSQL_ON_DEPLOY" == "1" ]]; then
        log "Running one-time SQLite -> MySQL data copy"
        # shellcheck disable=SC2086
        php artisan db:migrate-sqlite-to-mysql \
            --source-connection="$MIGRATE_SQLITE_SOURCE_CONNECTION" \
            --target-connection="$MIGRATE_SQLITE_TARGET_CONNECTION" \
            $MIGRATE_SQLITE_EXTRA_ARGS
    fi
fi
