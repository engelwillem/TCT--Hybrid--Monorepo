...
4. Quick endpoint check:
   - `curl http://127.0.0.1:8000/api/today/session`
   - Expected: root JSON includes `"contractVersion":"today.session.v1"`.

## 2) Frontend (Next.js)
1. Create/adjust `.env.local` in repo root:
   - `LARAVEL_API_BASE_URL=http://127.0.0.1:8000`
   - `TODAY_STRICT_INTEGRATION=true`
   - `TODAY_INTEGRATION_TRACE=true`
2. Start frontend:
   - `npm run dev`
3. Open:
   - `http://localhost:9002/today`

## 3) Expected behavior
- Page renders normally with ritual flow.
- Server logs contain `[today] content diagnostics` with `sourceStatus: "external"`.
...
- Set:
  - `TODAY_STRICT_INTEGRATION=false`
  - `TODAY_INTEGRATION_TRACE=false`
...
3. Frontend will forward `previewDate` to backend endpoint:
   - `/api/today/session?previewDate=2026-03-21`
