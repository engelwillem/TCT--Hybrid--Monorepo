...
function Assert-DeployEnvFiles {
    ...
    if ([string]::IsNullOrWhiteSpace($backendEnvFile) -or [string]::IsNullOrWhiteSpace($frontendEnvFile)) {
        throw "BACKEND_ENV_FILE and FRONTEND_ENV_FILE must be explicitly set for production migration."
    }

    if ($backendEnvFile -eq "backend-api/.env.docker" -or $frontendEnvFile -eq ".env.docker") {
        throw "Refusing migration with default docker env files. Set dedicated production env file paths."
    }
...
function Assert-ContractGuardrails {
    param([string]$Phrase)
    ...
    if ($Phrase -ne "APPROVE_CONTRACT_MIGRATION") {
        throw "Contract migration requires ContractConfirmPhrase=APPROVE_CONTRACT_MIGRATION."
    }
}
...
function Run-Migrations {
    ...
    switch ($Strategy) {
        ...
        "contract" {
            Assert-ContractGuardrails -Phrase $Phrase
            Invoke-BackendTask -Command "php artisan migrate --force"
        }
    }
}
...
Run-Migrations -Strategy $SchemaStrategy -Phrase $ContractConfirmPhrase

Write-Section "Run backfill"
Run-BackfillIfRequested -ShouldRun:$RunBackfill -Command $BackfillCommand

Write-Host "Production migration pipeline completed." -ForegroundColor Green
